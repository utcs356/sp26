...
### Overview
You will do your assignments for CS 356 using [CloudLab](http://cloudlab.us/) and [Kathara](https://www.kathara.org/).

* **CloudLab** is a research facility that provides bare-metal access and control over a substantial set of computing, storage, and networking resources. If you haven’t worked in CloudLab before, you need to register a CloudLab account.
* **Kathara** is a network emulation tool that enables you to test your network programs without multiple physical servers and network devices.
...
You should submit a report that shows you have successfully followed the process.
...
6. Here, please specify experiment details including name and project (It's normal if you do not see the `Group` option).
Feel free to choose your experiement `Name`, while `Project` should be `utcs356`.
Also, you need to specify from which cluster you want to start your experiment. We recommend you to first select the **Wisconsin** cluster. If it fails, then try another cluster **except for the Utah cluster**. Click `Next` to move to the next panel (`Schedule`).
...
**In your report, include the resource details from these commands.
For the CPU, please report the number of physical cores and logical cores (threads).
For the memory, please report the size of the available main memory.**
...
If you see any warnings or errors related to `xterm` while running the commands above, you can safely ignore them.

When prompted about the Kathara version, you may choose either option; however, we recommend using the pre-installed version.

```
A new version of image 'karthara/base' has been found on Docker Hub. Do you want to pull it? (Y/N) N
```
